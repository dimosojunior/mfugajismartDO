...
class GetDukaLakoSerializer(serializers.ModelSerializer):
    Status = UserStatusSerializer(many=False)
    comment_count = serializers.IntegerField(source='chat_messages.count', read_only=True)  # Add this line

    class Meta:
        model = DukaLako
        fields = '__all__'
        #fields = ['id', 'Title', 'Maelezo', 'PichaYaPost', 'Likes', 'Created', 'comment_count']  # Include comment_count in fields
...
#         model = ChatMessage
#         fields = '__all__'
#         #fields = ['id', 'sender', 'message', 'created_at', 'SenderImage', 'reply_to', 'replies']

#     def get_replies(self, obj):
#         replies = ChatMessage.objects.filter(reply_to=obj)
