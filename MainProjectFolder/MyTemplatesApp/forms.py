...
class MatatizoYaWatejaCreateForm(forms.ModelForm):
	JinaLaTatizo = forms.CharField(
		required=False,
	#label=False,
		widget=forms.TextInput(attrs={'id' :'Matatizo'})

	)

	Mkoa = forms.CharField(
		required=False,
	#label=False,
		widget=forms.TextInput(attrs={'id' :'Mkoa'})

	)
	

	class Meta:
		model = MatatizoYaWateja
		fields = '__all__'


class MatatizoYaWatejaSearchForm(forms.ModelForm):
